...
class class_get_values(object):
    ...
    def _debug(self, level, msg):
        if self.debug >= level:
            logging.debug("lib_maapi_onewire_pi_4_3 - {0}".format(msg))

    @classmethod
    def read_data_from_1w(self, rom_id, dev_id):
        try:
            w1_file = open('/sys/bus/w1/devices/{0}/w1_slave'.format(rom_id),
                           'r')
            self._debug(
                1, "Open file /sys/bus/w1/devices/{0}/w1_slave".format(rom_id))
        ...
        for rom_id in args:
            ...
            if condition:
                if condition_min_max:
                    ...
                    self._debug(
                        1,
                        "Condition min_max = {2}  \t Read value from sensor id = {0}, value is ={1}".
                        format(rom_id[0], value, condition_min_max))
                else:
                    maapidb.MaaPiDBConnection.insert_data(
                ...
                maapidb.MaaPiDBConnection.insert_data(rom_id[0], value, ' ',
                                                      True)
                self._debug(
                    1, "Read value for sensor id = {0}   is = {1} ".format(
                        rom_id[0], value))
