...
class class_get_values(object):
    ...
    def _debug(self, level, msg):
        if self.debug >= level:
            print("DEBUG OneWire_GPI0 {0} {1}, {2}".format(
    ...
    def __init__(self, *args):
        ...
        for arg in args:
            ...
            if mp_table[arg[0]]["switch_data_from_sens_id"] and mp_table[arg[0]]["switch_update_rom_id"]:
                self._debug(
                    1, "switch_data_from_sens_id and switch_update_rom_id  is TRUE")
                val_max = 1
                val_min = 1
                device_range_value = maapidb.MaaPiDBConnection().select_last_nr_of_values(
                    mp_table[arg[0]]["switch_data_from_sens_id"], mp_table[arg[0]]["switch_range_acc"])
                self._debug(1, "get switch_range_acc = {0}".format(
                    device_range_value))
                if mp_table[arg[0]]["switch_value_min_e"]:
                    if mp_table[arg[0]]["switch_reference_sensor_min_e"] and mp_table[arg[0]]["switch_reference_sensor_min_id"]:
                        ...
                        self._debug(
                            1, "switch_reference_sensor_min_e and switch_reference_sensor_min_id is True".format())
                        device_range_value_ref_min = maapidb.MaaPiDBConnection().select_last_nr_of_values(
                            mp_table[arg[0]]["switch_reference_sensor_min_id"], mp_table[arg[0]]["switch_range_acc"])

                        self._debug(1, "get device_range_value_ref_min = {0}".format(
                            device_range_value_ref_min))
                        for i in range(0, mp_table[arg[0]]["switch_range_acc"]):
                            ...
                            if device_range_value_ref_min[i] is not 99999 and device_range_value[i] is not 99999:
                                self._debug(2, "source {0} < ref {1} - val {2} = {3}".format(
                                                        ...
                                                        device_range_value_ref_min[i],
                                                        mp_table[arg[0]]["switch_value_min"],
                                                        device_range_value[i] < device_range_value_ref_min[i] - mp_table[arg[0]]["switch_value_min"]))
                        ...
                        self._debug(
                            1, "switch_reference_sensor_min_e and switch_reference_sensor_min_id is False".format())
